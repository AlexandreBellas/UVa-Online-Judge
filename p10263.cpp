...
public:
	...
	/*Angulo em forma de fracao reduzida entre o vetor Op (p Ã© o ponto)
	 e o eixo x, se paralelo ao eixo x retorna (1,0) ou (-1,0)
	 se paralelo ao eixo y retorna (0,1) ou (0,-1)
	 SÃ“ FUNCIONA PARA PONTOS INTEIROS*/ 
...
public:
	...
	OBS: se existe sÃ³ um ponto retorna 2 pontos iguais*/
...
int main(){
	...
	while(cin >> p.x >> p.y){
		...
		for(i = 1; i < N+1; i++){
			cin >> aux[i].x >> aux[i].y;
			respaux = p;
			if(p.distToLine(aux[i], aux[i-1], respaux) < dist){
				dist = p.distToLine(aux[i], aux[i-1], resp);
			}
		}

		cout << fixed << setprecision(4) << resp.x << endl;
		cout << fixed << setprecision(4) << resp.y << endl;
	}
